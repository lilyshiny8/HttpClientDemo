...
public class Request {
    ...
    protected void checkMethod(RequestMethod method) throws MethodNotSupportException {
        ...
        if (!RequestMethod.GET.equals(method) && !RequestMethod.HEAD.equals(method) && !RequestMethod.OPTIONS.equals(method)&&!RequestMethod.DELETE.equals(method)) {
	...
	public void addHeaders(String headers) {
		...
		for(int i=0;i<header.length;i++){
			String str[]=header[i].split(":", 2);
			addHeader(str[0],str[1]);
			RegularExtract.logger.info("添加请求头："+str[0]+"="+str[1]);
		}
	}
